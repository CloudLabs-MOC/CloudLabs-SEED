...
instead, it means adding bas the payload field of a and modifying the fields of a accordingly. As a result,
...
Sniffer programs can be easily written using the pcap library. Withpcap, the task of sniffers becomes
...
- Question 3. Please turn on and turn off the promiscuous mode in your sniffer program. The value
    1 of the third parameter in pcap_open_live()turns on the promiscuous mode (use 0 to turn it
    ...
    mode is on or off (look at the promiscuity’s value).
...
You may also need to use inet_addr(),inetnetwork(),inetntoa(),inet_aton() to
