...
The first page lets us see the current temperature setting of the the
(see Figure 2.a); it fetches the temperature value from the IoT server once every second. We should keep this page always visible, so we
...
http://www.attacker32.com to the IP address of the attacker’s web server, so the user can get the actual page
