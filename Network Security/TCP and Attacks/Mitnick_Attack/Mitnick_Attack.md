...
will usershin this task, we should know that rsh and rlogin programs are not secure, and they are
...
The rsh server program uses two files for authentication, .rhosts and /etc/hosts.equiv. Every
...
typing passwords, he created a .rhosts file on host X-Terminal and put the trusted server’s IP address
into the file. Note that the .rhosts file must reside at the top level of a user’s home directory and can be
...
a rsh session with X-Terminal. Sincershruns on top of TCP, we first need to establish a TCP connection
...
user, and if the user is authenticated,rshd initiates a separate TCP connection with the client.
...
![](Images/Second_Connection_internet.png)
...
connection is used by rshd to send out error messages. In our attack, we will not use this connection, but
if this connection is not established,rshd will stop without executing our command. Therefore, we need
...
If both connections have been successfully established,rshd will execute the command contained in
the rsh data packet. Please check the /tmpfolder and see whether /tmp/xyz is created and whether its
...
echo + + > .rhosts
```
Students should replace the touch command in Task 2 with the echo command above, and then
