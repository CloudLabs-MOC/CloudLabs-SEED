...
HostU), VPN server (the router/gateway), and a host in the private network (Host V). The network setup is
depicted in Figure 1.

![](Images/Network_Bind.png)
...
Host V to VPN Server (also serving as a gateway). In such a setup, HostVis not directly accessible from
...
Enter the Labsetup folder, and use the docker-compose.yml file to set up the lab environment. Detailed
...
operating system via a TUN/TAP network interface are delivered
to the user-space program. On the other hand, packets sent by the program via a TUN/TAP network interface are injected into the operating system
...
We will run the tun.py program on Host U. Make the above tun.py program executable, and run it using
...
field of the IP packet. Please use the following while loop to replace the one in tun.py:
...
- On Host U,pinga host in the 192.168.53.0/24 network. What are printed out by the tun.py
...
The server program **tunserver.py**. We will run tun_server.py program on VPN Server. This
...
Let’s rename it, and call it tun_client.py. Sending data to another computer using UDP can be done
using the standard socket programming.

Replace the while loop in the program with the following: The SERVER_IP and SERVER_PORT
...
Testing. Run the tun_server.py program on VPN Server, and then run tun_client.py on Host
...
be routed to the TUN interface and be given to the tun_client.py program. The following command
...
network, the ICMP packets are received by tun_server.py through the tunnel.

## 5 Task 4: Set Up the VPN Server

After tun_server.py gets a packet from the tunnel, it needs to feed the packet to the kernel, so the kernel
can route the packet towards its final destination. This needs to be done through a TUN interface, just like
what we did in Task 2. Please modify tun_server.py, so it can do the following:
...
Before running the modified tun_server.py, we need to enable the IP forwarding. Unless specifi-
...
After getting to this point, one direction of your tunnel is complete, i.e., we can send packets from Host U
to Host V via the tunnel. If we look at the Wireshark trace on HostV, we can see that HostVhas sent out
...
On Host U,telnet to Host V. While keeping the telnet connection alive, we break the VPN tunnel by
stopping the tun_client.py or tun_server.py program. We then type something in the telnet
window. Do you see what you type? What happens to the TCP connection? Is the connection broken?

Let us now reconnect the VPN tunnel (do not wait for too long). We will run the tun_client.py
and tun_server.py programs again, and set up their TUN interfaces and routing (this is where you can
find that including the configuration commands in the programs will make your life much easier). Once the
tunnel is re-established, what is going to happen to the telnet connection? Please describe and explain
your observations.
...
ure 2. The whole setup is described in the docker-compose2.yml file, and you can use the "-f
docker-compose2.yml" option to ask docker-compose to use this file, instead of the default
...
similar; the only difference is in the interface type. For TAP interfaces, we use IFF_TAP, while for TUN,
we use IFF_TUN. The rest of the code are the same, so we do not include them in the following. The way
...
and prints out all its fields. Try topingan IP address in the 192.168.53.0/24 network; report and
