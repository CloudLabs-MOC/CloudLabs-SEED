...
We can usesshto create a dynamic port-forwarding tunnel between B and A. We run the following com-
...
Generalpage, find the "Network Settings" section, click the Settings button, and a window
...
browsing traffic has gone through the tunnel, you should do the following: (1) run tcp dump on the router/-
...
The Local Command entry specifies the command running on the VPN client side. It configures the
client-side TUN interface: assigning the 192.168.53.88/24 address to the interface and bringing it
up. The Remote Command entry specifies the command running on the VPN server side. It configures the
