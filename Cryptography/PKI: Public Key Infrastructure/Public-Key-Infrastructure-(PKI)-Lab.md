...
- Chapter 23 of the SEED Book,_Computer & Internet Security: A Hands-on Approach_, 2nd Edition,
...
will not work):
```
10.9.0.80 http://www.bank32.com
10.9.0.80 http://www.smith2020.com
```
## 3 Lab Tasks

### 3.1 Task 1: Becoming a Certificate Authority (CA)

A Certificate Authority (CA) is a trusted entity that issues digital certificates. The digital certificate certifies
the ownership of a public key by the named subject of the certificate. A number of commercial CAs
...
The command to generate a CSR is quite similar to the one we used in creating the self-signed certificate
for the CA. The only difference is the -x509 option. Without it, the command generates a request;
with it, the command generates a self-signed certificate. The following command generate a CSR for
http://www.bank32.com(you) should use your own server name:
```
...
example.com, example.net, and example.org are all pointing to the same web server. Due to the
...
```
In the above command, myCAopenssl.cnf is the configuration file we copied from /usr/lib/ssl/openssl.cnf
(we also made changes to this file in Task 1). We use the policy anything
...
Copy the extension field. For security reasons, the default setting in openssl.cnf does not allow the
...
a website’s files are stored. This is done via its VirtualHost file, located in the /etc/apache2/sites-available
directory. In our container, we have a file called bank32apachessl.conf,
...
bank32 is dees. Once everything is set up properly, we can browse the web site, and all the traffic
