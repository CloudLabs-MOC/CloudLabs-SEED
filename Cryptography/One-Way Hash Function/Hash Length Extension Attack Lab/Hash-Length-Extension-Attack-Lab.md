...
Setting up the hostname. We use the domain www.seedlablenext.com to host the server program.
Since we only use one VM for this lab, we map this hostname to local host(127.0.0.1). This can be
...
The www directory contains the server code, and the LabHome directory contains a secret file and the
...
The server uses a Python module named Flask. By the time this lab is officially released, the VM
...
A typical request sent by the client to the server is shown below. The server requires a uid argument
to be passed. It uses uid to get the MAC key from LabHome/key.txt. The command in the example
...
Students should replace the value JohnDoein the myname field with their actual names (no space is
...
Students need to pick a uid number from thekey.txtin the LabHome directory. This file contains a list
...
bytes, so the padding is64 - 22 = 42bytes, including 8 bytes of the length field. The length of M in terms of bits is (^22) * 8 = 176 = 0xB0. SHA256 will be performed in the following padded message:
...
is 0x012345, the length field in the padding should be:
...
and <uid> should be obtained from the LabHome/key.txtfile.
...
In this task, we will add an extra message N = "Extra message"to the padded original message M =
...
and the MAC key should be obtained from the LabHome/key.txtfile. Please send this request to the
...
changing \x to %. For example, \x80 in the padding should be replaced with %80 in the URL above. On
...
key should be obtained from the LabHome/key.txtfile):
...
Based on the <mac> value calculated above, please construct a new request that includes the download
...
verify_mac() function and use Python’s hmac module to calculate the MAC. The function resides in
