...
- Chapter 23 of the SEED Book,_Computer & Internet Security: A Hands-on Approach,_ 2nd Edition,
...
The RSA algorithm involves computations on large numbers. These computations cannot be directly conducted
...
Let p,q, andebe three prime numbers. Let n = (p x q), We will use(e, n)as the public key. Please
calculate the private key d. The hexadecimal values of p,q, andeare listed in the following. It should be
...
using the hex-to-bn APIBNhex2bn(). The following python command can be used to convert a plain
...
c0.pem and the second onec1.pem.
...
**Step 3: Extract the signature from the server’s certificate.** There is no specific openssl command to
...
```
The field starting from (1) is the body of the certificate that is used to generate the hash; the field starting
from (2) is the signature block. Their offsets are the numbers at the beginning of the lines. In our case, the
