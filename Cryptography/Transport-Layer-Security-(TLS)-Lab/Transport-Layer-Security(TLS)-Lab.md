...
- Step 1: Get the IP address of www.example.com using thedigcommand, such as the following
...
the /usr/lib/ssldirectory. Inside this file, the copy_extensions option is disabled (commented
...
to the DNS servers configured on the host (i.e., the VM), and the host’s DNS server do use the /etc/hosts file. Therefore, due to the change above, on the proxy container, the IP address towww.example.comis
